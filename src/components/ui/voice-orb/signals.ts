...
): VoiceOrbSignal | undefined {
  if (!signal) return undefined;
  return {
    ...signal,
    volume:
      signal.volume === undefined ? undefined : clampVolume(signal.volume),
    inputVolume:
      signal.inputVolume === undefined
        ? undefined
        : clampVolume(signal.inputVolume),
    outputVolume:
      signal.outputVolume === undefined
        ? undefined
        : clampVolume(signal.outputVolume),
  };
}
...
export function deriveVoiceOrbVolume(
  ...
  if (state === "listening") {
    return clampVolume(
      signal?.inputVolume ?? signal?.volume ?? meteredVolume,
    );
  }
  if (state === "speaking") {
    return clampVolume(
      signal?.outputVolume ?? signal?.volume ?? meteredVolume,
    );
  }
