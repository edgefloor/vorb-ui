...
export function calibrateAudioLevel(
  ...
  const raw = Number.isFinite(rawValue) ? Math.min(1, Math.max(0, rawValue)) : 0;
  const noiseFloor = Math.min(1, Math.max(0, options.noiseFloor ?? 0.008));
  ...
  const shaped = Math.pow(Math.min(1, Math.max(0, gated * gain * options.sensitivity)), exponent);
  const rate = shaped > previous ? options.attack : options.release;
  return Math.min(1, Math.max(0, previous + (shaped - previous) * rate));
}

export function useAudioMeter(stream: MediaStream | null | undefined, options: AudioMeterOptions) {
  ...
  }, [stream, sensitivity, attack, release, noiseFloor, gain, exponent]);
