...
import { FRAGMENT_SHADER, VERTEX_SHADER } from "./crystal-ball-shaders";
import { VOICE_ORB_SCALES, getVoiceOrbTransitionDuration } from "./voice-orb-scales";
import type {
  VoiceOrbMotion,
  VoiceOrbCloudMode,
...
type RgbToneRamp = Record<keyof VoiceOrbToneRamp, Rgb>;
...
const RAMP_KEYS: Array<keyof VoiceOrbToneRamp> = ["deepest", "deep", "base", "bright", "lightest"];
const VISUAL_KEYS: Array<keyof VoiceOrbVisualState> = [
  "turbulence",
  "flowSpeed",
  "vortexCount",
  "vortexStrength",
...
function hexRampToRgb(ramp: VoiceOrbToneRamp): RgbToneRamp {
  return Object.fromEntries(RAMP_KEYS.map((key) => [key, hexToRgb(ramp[key])])) as RgbToneRamp;
}

const FALLBACK_MAIN = hexRampToRgb(VOICE_ORB_SCALES.crystal.colors.main);
const FALLBACK_WARNING = hexRampToRgb(VOICE_ORB_SCALES.crystal.colors.warning);
...
function parseToneRamp(ramp: VoiceOrbToneRamp, fallback: RgbToneRamp): RgbToneRamp {
...
function approachVisualState(
  current: VoiceOrbVisualState,
  target: VoiceOrbVisualState,
  rate: number,
) {
