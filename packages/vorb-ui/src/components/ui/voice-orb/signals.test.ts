...
describe("voice orb signals", () => {
  it("uses scalar, signal, adapter, microphone, then idle state precedence", () => {
    expect(
      deriveVoiceOrbState("thinking", { state: "speaking" }, { state: "listening" }, "connecting"),
    ).toBe("thinking");
    expect(
      deriveVoiceOrbState(undefined, { state: "speaking" }, { state: "listening" }, "connecting"),
    ).toBe("speaking");
    expect(deriveVoiceOrbState(undefined, undefined, { state: "listening" }, "connecting")).toBe(
      "listening",
    );
    expect(deriveVoiceOrbState(undefined, undefined, undefined, "connecting")).toBe("connecting");
    expect(deriveVoiceOrbState(undefined, undefined, undefined, undefined)).toBe("idle");
  });
  ...
  it("chooses direction-aware volume with scalar precedence", () => {
    ...
    expect(deriveVoiceOrbVolume(undefined, "listening", signal, 0.1)).toBe(0.35);
    ...
    expect(deriveVoiceOrbVolume(undefined, "listening", undefined, 0.45)).toBe(0.45);
  });
